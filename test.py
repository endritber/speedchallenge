...
if __name__ == '__main__':
  ...
  while True:
    ...
    flow = calculate_opticalflow(previous_frame, current_frame, augmentation=False).transpose(2, 0, 1)
    print(flow)
    ...
    if SHOW:
      ...
      if not TEST:
        cv2.putText(current_frame, 'Actual Speed: ' + str(round(speed[i], 2)) + ' mph', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)	
        
      cv2.imshow('speednet model (press q to quit)', current_frame)  
      i+=1
